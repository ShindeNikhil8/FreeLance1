...
<div className="mt-16 grid gap-12 lg:grid-cols-2">
  ...
  />
        ...
        <div className="mt-24">
          ...
          <div className="grid gap-8 md:grid-cols-2 xl:grid-cols-2 items-stretch">
            {cncmachinery.map((machine) => (
              <Card
                key={machine.id}
                className="group flex flex-col md:flex-row items-start gap-6 p-6 md:p-8 h-full border border-slate-200 rounded-2xl bg-white shadow-sm transition hover:shadow-xl hover:border-orange-400 hover:-translate-y-1"
              >
                {/* Left Content */}
  ...
  />
          ...
          {/* VMC Machines */}
<h2 className="text-center text-3xl font-extrabold mt-16 mb-8 text-slate-900">
  VMC (Vertical Machining Centre)
</h2>

<div className="grid gap-8 md:grid-cols-2 items-stretch">
  {vmcmachinery.map((machine, index) => (
    <div
      key={machine.id}
      className={
        vmcmachinery.length % 2 !== 0 &&
        index === vmcmachinery.length - 1
